...
const JourneyMap = ({ journey }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setIsloading(true);
                const depRes = await fetchStation(journey.departureStationId);
                const retRes = await fetchStation(journey.returnStationId);

                setDepartureCoordinates([depRes.y, depRes.x])
                setReturnCoordinates([retRes.y, retRes.x])
                setIsloading(false);
            } catch (error) {
